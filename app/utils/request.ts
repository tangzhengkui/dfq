...
export function setToken (token: string,tokenName:string='token') {
  axios.defaults.headers.common['token'] = ``;
  localStorage.setItem('TOKEN', token)
  localStorage.setItem('TOKEN_EXPIRE', `${new Date().getTime() + tokenExpired}`)
  axios.defaults.headers.common[tokenName] = `Bearer ${token}`
}
...
function syncToken (e: StorageEvent) {
  ...
  if (!newValue) {
    ...
    axios.defaults.headers.common['token'] = `Bearer ${newValue}`
  }
}
